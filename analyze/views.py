...
def ward_level_analysis(request):
    ward_user = acc_models.WardUser.objects.get(
...
def cluster_level_analysis(request):
    ...
    for ward in ward_list:
        ...
        data[0].append(parent_models.Vaccine.objects.all().filter(
            child__parent__address__in=locality_list, date__lte=datetime.now(), status__exact=True).count())
        data[1].append(parent_models.Vaccine.objects.all().filter(
            child__parent__address__in=locality_list, date__lte=datetime.now(), status__exact=False).count())
...
def district_level_analysis(request):
    district_user = acc_models.DistrictUser.objects.get(
        user__username__exact=request.user.username)
